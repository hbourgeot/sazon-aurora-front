...
import { chevron_left, chevron_right } from 'stwui/icons';
...
export let onPageClick: (pageNumber: number) => void;
export let pageSize = 25;

function pageRange(cp: number, tp: number) {
    if (tp <= 6) {
			...
			<nav class="isolate inline-flex -space-x-px rounded-md shadow-md">
				...
				{#each pagination as pageNumber}
					...
					{:else if typeof pageNumber === 'number'}
						<button
							type="button"
							on:click={() => onPageClick(Number(pageNumber))}
